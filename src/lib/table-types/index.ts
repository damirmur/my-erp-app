...
export async function syncTableTypes() {
    ...
    for (const row of data) {
        ...
        loaded[row.name] = {
            ...
            features: (def.features ?? { hierarchy: false, copy: false, print: false, tabularSections: false }) as TableTypeFeatures,
            actions: (def.actions ?? []).map((a: ActionDefDB) => ({
                ...a,
                show: a.showWhen ? (s: string) => s === a.showWhen : a.showWhenNot ? (s: string) => s !== a.showWhenNot : undefined,
...
export async function saveTableTypeToDB(name: string, label: string, module: TableTypeModule) {
    ...
    const { error } = await supabase.from('meta_table_types').upsert(
